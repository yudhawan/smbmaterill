...
function Login() {
    ...
    function handleSubmit(e){
        ...
        if(email.length===0 || password.length===0){
            ...
            dispatch(login({email:email,password:password,role:'materill'}))
        }
    }
  ...
  return (
    ...
    <div className='w-full h-screen flex flex-col space-y-4 justify-center items-center -mt-32'>
        <div className="flex flex-col justify-center">
            <p className="text-indigo-500 font-bold text-6xl">Materill</p>
        ...
        <form onSubmit={handleSubmit} className='flex flex-col rounded-lg border border-slate-400 w-80 h-auto space-y-1 p-2'>
            ...
            <button className='bg-indigo-500 text-white py-1 px-2 rounded-lg w-fit' onClick={handleSubmit}>{loading?<>Loading</>:<>Login</>}</button>
        </form>
    </div>}
    </>
  )
